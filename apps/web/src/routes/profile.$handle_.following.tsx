...
export const Route = createFileRoute("/profile/$handle_/following")({
  ...
  loader: async ({ params, deps }) => {
    ...
    const list = await getFollowList(params.handle, "following", {
      page: deps.page,
      limit: PAGE_SIZE,
    });
    return { profile, list };
...
function FollowingPage() {
  ...
  const infinite = useInfinitePagination({
    ...
    resetKey: `${profile.did}:following`,
    initialPage: list,
    ...
    loadPage: (target) =>
      getFollowList(profile.did, "following", { page: target, limit: PAGE_SIZE }),
  });
